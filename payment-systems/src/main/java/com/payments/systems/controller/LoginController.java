...
public class LoginController {
    ...
    @PostMapping("/login")
    public ResponseEntity<OutputMetaData> loginUser(@Valid @RequestBody LoginRequest loginRequest) {
    return userSignInService.loginUser(loginRequest);
    }

    @PostMapping("/signup")
    public ResponseEntity<OutputMetaData> signup(@Valid @RequestBody User user) {
